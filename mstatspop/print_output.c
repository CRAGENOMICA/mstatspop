...
{
    int outgroup_presence = args->outgroup_presence + args->force_outgroup;
    int x = 0;
    int y = 0;
    ...
    {
        initsq1[x] = sumnsam;
        sumnsam += args->vint_perpop_nsam[x];
    }
    ...
    if (args->output == 0 || args->output == 10)
    {
        ...
        if (args->gfffiles == 1)
        {
            fprintf(file_out, "\nGFF file: %s", args->file_GFF);
            fprintf(file_out, "\nPositions studied: %s", args->subset_positions);
            if (strcmp(args->subset_positions, "synonymous") == 0 || strcmp(args->subset_positions, "nonsynonymous") == 0 || strcmp(args->subset_positions, "silent") == 0)
                fprintf(file_out, "\nGenetic Code: %s", args->code_name);
            if (strcmp(args->code_name, "Other") == 0)
            {
                fputs("\nGenetic Code assigned to each triplet:\n", file_out);
                for (x = 0; x < 64; x++)
                {
                    fprintf(file_out, "[%c%c%c] = %c\n", tripletsU[x][0], tripletsU[x][1], tripletsU[x][2], args->genetic_code[x]);
                }
            }
        }
        fprintf(file_out, "\n\nInclude Unknown positions [1/0]: %d", args->include_unknown);
        ...
        if (args->include_unknown == 1)
            fprintf(file_out, "Ratio_Missing (if outgroup, counting only positions where outgroup is present): %.6f", missratio);
        else
            fprintf(file_out, "Ratio_Missing (if outgroup, counting only positions where outgroup is present): NA");
        
        fprintf(file_out, "\n\nNumber of populations: %d", args->npops - !outgroup_presence);
        if (ploidy[0] == '2')
            fprintf(file_out, "\nNumber of samples (duplicated) for each population:");
        
        fprintf(file_out, "\nNumber of samples for each population:");
        for (x = 0; x < args->npops - !outgroup_presence; x++)
        {
            fprintf(file_out, "\nnsam[%d]: %d", x, args->vint_perpop_nsam[x]);
        }
        /*if(missratio > 0.) {*/
        /*
        ...
        for (x = 0; x < args->npops - 1; x++)
        {
            ...
            if (outgroup_presence + args->force_outgroup)
            {
            ...
            if (outgroup_presence + args->force_outgroup == 0)
            {
        ...
        if(args->include_rsfs==0) {
                ...
                if (outgroup_presence == 1)
                {
                    ...
                    for (x = 0; x < np; x++)
                    {
                        if (args->vint_perpop_nsam[x] > 1)
                        {
                             ...
                             */
                            fprintf(file_out, "S[%d]: %d\t", x, (int)statistics[0].So[x]);
                            if (nsites2_pop_outg[x] > 0)
                                fprintf(file_out, "Theta/nt(Wat)[%d]: %f\t", x, statistics[0].thetaSo[x] / (double)nsites2_pop_outg[x]);
                            else
                                fprintf(file_out, "Theta/nt(Wat)[%d]: NA\t", x);
                            if (nsites2_pop_outg[x] > 0)
                                fprintf(file_out, "Theta/nt(Taj)[%d]: %f\t", x, statistics[0].thetaTo[x] / (double)nsites2_pop_outg[x]);
                            else
                                fprintf(file_out, "Theta/nt(Taj)[%d]: NA\t", x);
                            if (nsites2_pop_outg[x] > 0)
                                fprintf(file_out, "Theta/nt(Fu&Li)[%d]: %f\t", x, statistics[0].thetaFL[x] / (double)nsites2_pop_outg[x]);
                            else
                                fprintf(file_out, "Theta/nt(Fu&Li)[%d]: NA\t", x);
                            if (nsites2_pop_outg[x] > 0)
                                fprintf(file_out, "Theta/nt(Fay&Wu)[%d]: %f\t", x, statistics[0].thetaFW[x] / (double)nsites2_pop_outg[x]);
                            else
                                fprintf(file_out, "Theta/nt(Fay&Wu)[%d]: NA\t", x);
                            if (nsites2_pop_outg[x] > 0)
                                fprintf(file_out, "Theta/nt(Zeng)[%d]: %f\t", x, statistics[0].thetaL[x] / (double)nsites2_pop_outg[x]);
                            else
                                fprintf(file_out, "Theta/nt(Zeng)[%d]: NA\t", x);
                            if (nsites3_pop_outg[x] > 0)
                                fprintf(file_out, "Theta/nt(Achaz,Wat)[%d]: %f\t", x, statistics[0].thetaSA[x] / (double)nsites3_pop_outg[x]);
                            else
                                fprintf(file_out, "Theta/nt(Achaz,Wat)[%d]: NA\t", x);
                            if (nsites3_pop_outg[x] > 0)
                                fprintf(file_out, "Theta/nt(Achaz,Taj)[%d]: %f\t", x, statistics[0].thetaTA[x] / (double)nsites3_pop_outg[x]);
                            else
                                fprintf(file_out, "Theta/nt(Achaz,Taj)[%d]: NA\t", x);
                            if (nsites1_pop_outg[x] > 0)
                                fprintf(file_out, "Divergence[%d]: %f\t", x, statistics[0].K[x] / (double)nsites1_pop_outg[x]);
                            else
                                fprintf(file_out, "Divergence/nt[%d]: NA\t", x);
                            if (statistics[0].thetaTHKY[x] > -10000 /* && missratio == 0.*/)
                                fprintf(file_out, "Theta/nt(Taj)HKY[%d]: %f\t", x, statistics[0].thetaTHKY[x]);
                            else
                                fprintf(file_out, "Theta/nt(Taj)HKY[%d]: NA\t", x);
                            if (statistics[0].KHKY[x] > -10000 /* && missratio == 0.*/)
                                fprintf(file_out, "Divergence/nt_HKY[%d]: %f\n", x, statistics[0].KHKY[x]);
                            else
                ...
                else
                {
                    ...
                    for (x = 0; x < np; x++)
                    {
                        if (args->vint_perpop_nsam[x] > 1)
                        {
                            fprintf(file_out, "S[%d]: %d\t", x, (int)statistics[0].S[x]);
                            if (nsites2_pop[x] > 0)
                                fprintf(file_out, "Theta/nt(Wat)[%d]: %f\t", x, statistics[0].thetaS[x] / (double)nsites2_pop[x]);
                            else
                                fprintf(file_out, "Theta/nt(Wat)[%d]: NA\t", x);
                            if (nsites2_pop[x] > 0)
                                fprintf(file_out, "Theta/nt(Taj)[%d]: %f\t", x, statistics[0].thetaT[x] / (double)nsites2_pop[x]);
                            else
                                fprintf(file_out, "Theta/nt(Taj)[%d]: NA\t", x);
                            if (nsites2_pop[x] > 0)
                                fprintf(file_out, "Theta/nt(Fu&Li)[%d]: %f\t", x, statistics[0].thetaFL[x] / (double)nsites2_pop[x]);
                            else
                                fprintf(file_out, "Theta/nt(Fu&Li)[%d]: NA\t", x);
                            if (nsites3_pop[x] > 0)
                                fprintf(file_out, "Theta/nt(Achaz,Wat)[%d]: %f\t", x, statistics[0].thetaSA[x] / (double)nsites3_pop[x]);
                            else
                                fprintf(file_out, "Theta/nt(Achaz,Wat)[%d]: NA\t", x);
                            if (nsites3_pop[x] > 0)
                                fprintf(file_out, "Theta/nt(Achaz,Taj)[%d]: %f\t", x, statistics[0].thetaTA[x] / (double)nsites3_pop[x]);
                            else
                                fprintf(file_out, "Theta/nt(Achaz,Taj)[%d]: NA\n", x);
                            if (statistics[0].thetaTHKY[x] > -10000 /* && missratio == 0.*/)
                                fprintf(file_out, "Theta/nt(Taj)HKY[%d]: %f\t", x, statistics[0].thetaTHKY[x]);
                            else
                                fprintf(file_out, "Theta/nt(Taj)HKY[%d]: NA\t", x);
            ...
            if (outgroup_presence == 1)
            {
                ...
                for (x = 0; x < np; x++)
                {
                    ...
                    if (ploidy[0] == '1')
                    {
                       ...
                       */
                    for (xx = 0; xx < args->r2i_ploidies[0]; xx++)
                    {
                        if (statistics[0].R2p[xx][x] > -10000)
                        {
                            if (args->r2i_ploidies[0] == 1 && ploidy[0] == '1')
                                fprintf(file_out, "R2[%d]: %f\t", x, statistics[0].R2p[xx][x]);
                            if (args->r2i_ploidies[0] == 1 && ploidy[0] == '2')
                                fprintf(file_out, "R2d[%d]: %f\t", x, statistics[0].R2p[xx][x]);
                            if (args->r2i_ploidies[0] != 1)
                                fprintf(file_out, "R2p%d[%d]: %f\t", args->r2i_ploidies[xx + 1], x, statistics[0].R2p[xx][x]);
                        }
                        else
                        {
                            if (args->r2i_ploidies[0] == 1 && ploidy[0] == '1')
                                fprintf(file_out, "R2[%d]: NA\t", x);
                            if (args->r2i_ploidies[0] == 1 && ploidy[0] == '2')
                                fprintf(file_out, "R2d[%d]: NA\t", x);
                            if (args->r2i_ploidies[0] != 1)
                                fprintf(file_out, "R2p%d[%d]: NA\t", args->r2i_ploidies[xx + 1], x);
                        }
                    }
            ...
            if (args->npops > 2 && outgroup_presence == 1)
            {
                for (x = 0; x < args->npops - 1; x++)
                {
                ...
                fprintf(file_out, "Sx[outg]: %ld\t", statistics[0].Sanc[(args->npops - 1) * 4 + 0]);
                fprintf(file_out, "Sf[outg]: %ld\t", statistics[0].Sanc[(args->npops - 1) * 4 + 1]);
                fprintf(file_out, "Ss[outg,rest]: %ld\t", statistics[0].Sanc[(args->npops - 1) * 4 + 2]);
                if (args->npops > 3)
                    fprintf(file_out, "Ss[rest]:\t%ld\n", statistics[0].Sanc[(args->npops - 1) * 4 + 3]);
                else
                    fprintf(file_out, "Ss[rest]:\tNA\n");
            }
            else
            {
                if (args->npops > 2 && outgroup_presence == 0)
                {
                    for (x = 0; x < args->npops - 1; x++)
                    {
            ...
            for (x = 0; x < args->npops - 1 /*!outgroup_presence*/; x++)
            {
                if (args->vint_perpop_nsam[x] > 1)
                {
                    if (statistics[0].mdsd[x] > -10000.)
                        fprintf(file_out, "SDev[%d]: %f\t", x, statistics[0].mdsd[x]);
                    else
                        fprintf(file_out, "SDev[%d]: NA\t", x);
                }
                else
                    fprintf(file_out, "SDev[%d]: NA\t", x);
                if (args->vint_perpop_nsam[x] > 2)
                {
                    if (statistics[0].mdg1[x] > -10000. && statistics[0].mdsd[x] > -10000.)
                        fprintf(file_out, "Skewness[%d]: %f\t", x, statistics[0].mdg1[x]);
                    else
                ...
                if (args->vint_perpop_nsam[x] > 3)
                {
                    if (statistics[0].mdg2[x] > -10000. && statistics[0].mdsd[x] > -10000.)
                        fprintf(file_out, "Kurtosis[%d]: %f\t", x, statistics[0].mdg2[x]);
                    else
            ...
            if (args->npops - 1 /*!outgroup_presence*/ > 1)
            {
                ...
                if (args->npops - 1 /*!outgroup_presence*/ > 2)
                {
                    ...
                    if (ploidy[0] == '1' && args->include_unknown == 0)
                    {
                        if (statistics[0].fsthALL > -10000)
                        {
                            if (args->niter && piter[0].niterihall > 0 && args->include_unknown == 0)
                    ...
                    for (x = 0; x < args->npops - 1; x++)
                    {
                        ...
                        {
                            if (args->niter && piter[0].niteri1[x] > 0 && args->include_unknown == 0)
                        ...
                        {
                            if (args->niter && piter[0].niteri1[x] > 0)
                        ...
                        if (ploidy[0] == '1' && args->include_unknown == 0)
                        {
                            ...
                            {
                                if (args->niter && piter[0].niterih1[x] > 0 && args->include_unknown == 0)
                            ...
                            {
                                if (args->niter && piter[0].niterih1[x] > 0)
                ...
                for (x = 0; x < args->npops - 1; x++)
                {
                    for (y = x + 1; y < args->npops - 0; y++)
                    {
                        ...
                        {
                            if (args->niter && piter[0].niteri[z] > 0 && args->include_unknown == 0)
                ...
                { /*NOTE THE HAPLOTYPE VALUES GOES FROM 0 to npops-1 BUT FREQUENCY GOES FROM 0 to npops!!!!!!*/
                    for (y = x + 1; y < args->npops - 0; y++)
                    {
                        ...
                        if (ploidy[0] == '1' && args->include_unknown == 0)
                        {
                            ...
                            {
                                if (args->niter && piter[0].niterih[z] > 0 && args->include_unknown == 0)
                ...
                if (args->include_unknown == 0 && outgroup_presence == 1)
                {
                            ...
                            for (y = x + 1; y < args->npops - 0; y++)
                            {
                                ...
                                else
                                {
                                    ...
                                    if (statistics[0].piaHKY[z] > -10000)
                                    {
                                        ...
                                        fprintf(file_out, "PiAHKY[%d,%d]: NA\tPiTHKY[%d,%d]: NA\t", x, y, x, y);
                                }
                                fprintf(file_out, "\n");
                                z++;
                            }
            ...
            if (args->include_unknown == 0)
            {
                ...
                for (x = 0; x < args->npops - 1 /*!outgroup_presence*/; x++)
                {
                    if (args->vint_perpop_nsam[x] > 1)
                    {
                        ...
                        if (ploidy[0] == '1' && args->include_unknown == 0)
                            fprintf(file_out, "HapW[%d]: %f\t", x, statistics[0].hapw[x]);
                        fprintf(file_out, "\n");
                    }
                    else
                    {
            ...
            if (args->npops - !outgroup_presence > 1)
            {
                for (x = 0; x < args->npops - 1 /*!outgroup_presence*/; x++)
                {
                    fprintf(file_out, "\n");
                    ...
                    {
                        for (y = 1; y <= floor(args->vint_perpop_nsam[x] / 2); y++)
                        {
                            fprintf(file_out, "fr[%d,%d]: %ld\t", x, y, statistics[0].freq[x][y]);
                        }
                    }
            ...
            {
                fprintf(file_out, "\n");
                for (y = 1; y <= floor(args->vint_perpop_nsam[0] / 2); y++)
                {
            ...
            if (ploidy[0] == '1' && args->include_unknown == 0)
            {
                ...
                {
                    fprintf(file_out, "\n");
                    for (y = 0; y < statistics[0].nh; y++)
                    { ///******* problem!: in case force an outgroup we may have one more haplotype that is not real *********/
            ...
            if (args->npops - !outgroup_presence > 1)
            {
                for (x = 0; x < args->npops - 1; x++)
                {
                    fprintf(file_out, "\nSx[%d]: ", x);
                    y = 0;
                    zz = sites_matrix[y * 4 * args->npops + 4 * x + 0];
                    while (zz != 0)
                    {
                        if (y >= length_seg)
                            break;
                        ...
                        {
                            if (zz < 0)
                                fprintf(file_out, "%ld ", zz - (long int)vector_priors[0] + 1);
                            else
                                fprintf(file_out, "%ld ", zz + (long int)vector_priors[0] - 1);
                        }
                        else
                            fprintf(file_out, "%ld ", zz);
                        y++;
                        zz = sites_matrix[y * 4 * args->npops + 4 * x + 0];
                    }
                    ...
                    {
                        y = 0;
                        zz = sites_matrix[y * 4 * args->npops + 4 * x + 2];
                        while (zz != 0)
                        {
                            if (y >= length_seg)
                                break;
                            if (args->formatfile == 3)
                            {
                                if (zz < 0)
                                    ...
                                    fprintf(file_out, "%ld ", zz + (long int)(long int)vector_priors[0] - 1);
                            }
                            else
                                fprintf(file_out, "%ld ", zz);
                            y++;
                    ...
                    fprintf(file_out, "\nSf[%d]: ", x);
                    y = 0;
                    zz = sites_matrix[y * 4 * args->npops + 4 * x + 1];
                    while (zz != 0)
                    {
                        if (y >= length_seg)
                            break;
                    ...
                    fprintf(file_out, "\nSs[%d]: ", x);
                    y = 0;
                    zz = sites_matrix[y * 4 * args->npops + 4 * x + 3];
                    while (zz != 0)
                    {
                        if (y >= length_seg)
                            break;
                ...
                if (ploidy[0] == '1')
                {
                    ...
                    if (outgroup_presence == 0)
                    {
                        for (x = 0; x < args->npops - oo; x++)
                        {
                            ...
                            {
                                fprintf(file_out, "\nPop[%d]\n\t", x);
                                for (ss = initsq1[x]; ss < initsq1[x] + args->vint_perpop_nsam[x]; ss++)
                                    fprintf(file_out, "line[%d]\t", ss - initsq1[x]);
                                fprintf(file_out, "\n");
                                for (z1 = 1; z1 <= (long int)floor(args->vint_perpop_nsam[x] / 2); z1++)
                                {
                        ...
                        for (x = 0; x < args->npops - oo; x++)
                        {
                            if (args->vint_perpop_nsam[x] > 1)
                            {
                                ...
                                for (ss = initsq1[x]; ss < initsq1[x] + args->vint_perpop_nsam[x]; ss++)
                                    fprintf(file_out, "line[%d]\t", ss - initsq1[x]);
                                fprintf(file_out, "\n");
                                for (z1 = 1; z1 < args->vint_perpop_nsam[x]; z1++)
                                {
                                    fprintf(file_out, "freq[%d]:\t", z1);
                                    for (ss = initsq1[x]; ss < initsq1[x] + args->vint_perpop_nsam[x]; ss++)
                                    {
        ...
        if(args->include_rsfs==1) {
            fprintf(file_out, "\n\nRELATIVE Effective length for each population (with at least one sequence, two or three sequences per pop) and excluding or including outgroup (if defined):");
            for (x = 0; x < args->npops - 1; x++)
            {
                if (outgroup_presence + args->force_outgroup)
                {
                    fprintf(file_out, "\nrEff_length1_pop_outg[%d]: %.2f\trEff_length2_pop_outg[%d]: %.2f\trEff_length3_pop_outg[%d]: %.2f", x, (double)rnsites1_pop_outg[x], x, (double)rnsites2_pop_outg[x], x, (double)rnsites3_pop_outg[x]);
                }
                if (outgroup_presence + args->force_outgroup == 0)
                {
                    fprintf(file_out, "\nrEff_length1_pop[%d]: %.2f\trEff_length2_pop[%d]: %.2f\trEff_length3_pop[%d]: %.2f", x, (double)rnsites1_pop[x], x, (double)rnsites2_pop[x], x, (double)rnsites3_pop[x]);
                }
             }
            fprintf(file_out, "\n");
            /*PRINT relative VARIABILITY*/
            if (outgroup_presence + args->force_outgroup)
            {
                fprintf(file_out, "\nEstimate of Divergence versus outgroup:\n");
                fprintf(file_out, "Divergence: %f\t", statistics[0].K[0]);
                fprintf(file_out, "\n");
                ...
                for (x = 0; x < np; x++)
                {
                    ...
                    {
                        fprintf(file_out, "S[%d]: %d\t", x, (int)statistics[0].So[x]);
                        fprintf(file_out, "Theta(Taj)[%d]: %f\t", x, statistics[0].piw[x]);
                    }
                    else
                    {
                        fprintf(file_out, "S[%d]: NA\t", x);
                ...
                fprintf(file_out, "\nEstimates of RELATIVE variability for each population:\n");
                np = args->npops - 1;
                for (x = 0; x < np; x++)
                {
                    if (args->vint_perpop_nsam[x] && args->int_total_nsam> 1)
                    {
                        fprintf(file_out, "rS[%d]: %.0f\t", x, statistics[0].rS[x]);
                        fprintf(file_out, "rTheta(Wat)[%d]: %f\t", x, statistics[0].rthetaSo[x]);
                        fprintf(file_out, "rTheta(Taj)[%d]: %f\t", x, statistics[0].rthetaTo[x]);
                        fprintf(file_out, "rTheta(Fu&Li)[%d]: %f\t", x, statistics[0].rthetaFL[x]);
                        fprintf(file_out, "rTheta(Fay&Wu)[%d]: %f\t", x, statistics[0].rthetaFW[x]);
                        fprintf(file_out, "rTheta(Zeng)[%d]: %f\t", x, statistics[0].rthetaL[x]);
                        fprintf(file_out, "rTheta(Achaz,Wat)[%d]: %f\t", x, statistics[0].rthetaSA[x]);
                        fprintf(file_out, "rTheta(Achaz,Taj)[%d]: %f\t", x, statistics[0].rthetaTA[x]);
                        fprintf(file_out, "\n");
                    }
                    else
                    {
                        fprintf(file_out, "rS[%d]: NA\t", x);
                        fprintf(file_out, "rTheta(Wat)[%d]: NA\t", x);
                        fprintf(file_out, "rTheta(Taj)[%d]: NA\t", x);
                        fprintf(file_out, "rTheta(Fu&Li)[%d]: NA\t", x);
                        fprintf(file_out, "rTheta(Fay&Wu)[%d]: NA\t", x);
                        fprintf(file_out, "rTheta(Zeng)[%d]: NA\t", x);
                        fprintf(file_out, "rTheta(Achaz,Wat)[%d]: NA\t", x);
                        fprintf(file_out, "rTheta(Achaz,Taj)[%d]: NA\t", x);
                        fprintf(file_out, "\n");
                    }
                }
            }
            else
            {
                ...
                for (x = 0; x < np; x++)
                {
                    ...
                    {
                        fprintf(file_out, "rS[%d]: %.0f\t", x, statistics[0].rS[x]);
                        fprintf(file_out, "rTheta(Wat)[%d]: %f\t", x, statistics[0].rthetaS[x]);
                        fprintf(file_out, "rTheta(Taj)[%d]: %f\t", x, statistics[0].rthetaT[x]);
                        fprintf(file_out, "rTheta(Fu&Li)[%d]: %f\t", x, statistics[0].rthetaFL[x]);
                        fprintf(file_out, "rTheta(Achaz,Wat)[%d]: %f\t", x, statistics[0].rthetaSA[x]);
                        fprintf(file_out, "rTheta(Achaz,Taj)[%d]: %f\t", x, statistics[0].rthetaTA[x]);
                        fprintf(file_out, "\n");
            ...
            if (args->formatfile != 2)
            {
                ...
                if (outgroup_presence == 1)
                {
                    ...
                    double len = 0.;
                    for (x = 0; x < np; x++) {
                        len += (double)nsites1_pop_outg[x];
                    }
                    len /= (double)np;
                    
                    fprintf(file_out, "\nEstimate of Divergence/nt versus outgroup:\n");
                    fprintf(file_out, "Divergence/nt: %f\t", statistics[0].K[0]/len);
                    fprintf(file_out, "\n");
                    
                    fprintf(file_out, "\nEstimates of variability/nt for each population:\n");
                    ...
                    for (x = 0; x < np; x++)
                    {
                        if (args->vint_perpop_nsam[x] > 1)
                        {
                            fprintf(file_out, "S[%d]: %d\t", x, (int)statistics[0].So[x]);
                            if (nsites2_pop_outg[x] > 0)
                                fprintf(file_out, "Theta/nt(Taj)[%d]: %f\t", x, statistics[0].piw[x] / (double)nsites2_pop_outg[x]);
                            else
                                fprintf(file_out, "Theta/nt(Taj)[%d]: NA\t", x);
                        }
                        else {
                    ...
                    fprintf(file_out, "\nEstimates of RELATIVE variability/nt for each population:\n");
                    
                    for (x = 0; x < np; x++)
                    {
                        if (args->vint_perpop_nsam[x]  && args->int_total_nsam > 1)
                        {
                            fprintf(file_out, "rS[%d]: %.0f\t", x, statistics[0].rSo[x]);
                            if (rnsites2_pop_outg[x] > 0)
                                fprintf(file_out, "rTheta/nt(Wat)[%d]: %f\t", x, statistics[0].rthetaSo[x] / (double)rnsites2_pop_outg[x]);
                            else
                                fprintf(file_out, "rTheta/nt(Wat)[%d]: NA\t", x);
                            if (rnsites2_pop_outg[x] > 0)
                                fprintf(file_out, "rTheta/nt(Taj)[%d]: %f\t", x, statistics[0].rthetaTo[x] / (double)rnsites2_pop_outg[x]);
                            else
                                fprintf(file_out, "rTheta/nt(Taj)[%d]: NA\t", x);
                            if (rnsites2_pop_outg[x] > 0)
                                fprintf(file_out, "rTheta/nt(Fu&Li)[%d]: %f\t", x, statistics[0].rthetaFL[x] / (double)rnsites2_pop_outg[x]);
                            else
                                fprintf(file_out, "rTheta/nt(Fu&Li)[%d]: NA\t", x);
                            if (rnsites2_pop_outg[x] > 0)
                                fprintf(file_out, "rTheta/nt(Fay&Wu)[%d]: %f\t", x, statistics[0].rthetaFW[x] / (double)rnsites2_pop_outg[x]);
                            else
                                fprintf(file_out, "rTheta/nt(Fay&Wu)[%d]: NA\t", x);
                            if (rnsites2_pop_outg[x] > 0)
                                fprintf(file_out, "rTheta/nt(Zeng)[%d]: %f\t", x, statistics[0].rthetaL[x] / (double)rnsites2_pop_outg[x]);
                            else
                                fprintf(file_out, "rTheta/nt(Zeng)[%d]: NA\t", x);
                            if (rnsites3_pop_outg[x] > 0)
                                fprintf(file_out, "rTheta/nt(Achaz,Wat)[%d]: %f\t", x, statistics[0].rthetaSA[x] / (double)rnsites3_pop_outg[x]);
                            else
                                fprintf(file_out, "rTheta/nt(Achaz,Wat)[%d]: NA\t", x);
                            if (rnsites3_pop_outg[x] > 0)
                                fprintf(file_out, "rTheta/nt(Achaz,Taj)[%d]: %f\t", x, statistics[0].rthetaTA[x] / (double)rnsites3_pop_outg[x]);
                            else
                                fprintf(file_out, "rTheta/nt(Achaz,Taj)[%d]: NA\t", x);
                        }
                        else
                        {
                            fprintf(file_out, "rS[%d]: NA\t", x);
                ...
                else
                {
                    ...
                    for (x = 0; x < np; x++)
                    {
                        if (args->vint_perpop_nsam[x]  && args->int_total_nsam > 1)
                        {
                            fprintf(file_out, "rS[%d]: %.0f\t", x, statistics[0].rS[x]);
                            if (rnsites2_pop[x] > 0)
                                fprintf(file_out, "rTheta/nt(Wat)[%d]: %f\t", x, statistics[0].rthetaS[x] / (double)rnsites2_pop[x]);
                            else
                                fprintf(file_out, "rTheta/nt(Wat)[%d]: NA\t", x);
                            if (rnsites2_pop[x] > 0)
                                fprintf(file_out, "rTheta/nt(Taj)[%d]: %f\t", x, statistics[0].rthetaT[x] / (double)rnsites2_pop[x]);
                            else
                                fprintf(file_out, "rTheta/nt(Taj)[%d]: NA\t", x);
                            if (rnsites2_pop[x] > 0)
                                fprintf(file_out, "rTheta/nt(Fu&Li)[%d]: %f\t", x, statistics[0].rthetaFL[x] / (double)rnsites2_pop[x]);
                            else
                                fprintf(file_out, "rTheta/nt(Fu&Li)[%d]: NA\t", x);
                            if (rnsites3_pop[x] > 0)
                                fprintf(file_out, "rTheta/nt(Achaz,Wat)[%d]: %f\t", x, statistics[0].rthetaSA[x] / (double)rnsites3_pop[x]);
                            else
                                fprintf(file_out, "rTheta/nt(Achaz,Wat)[%d]: NA\t", x);
                            if (rnsites3_pop[x] > 0)
                                fprintf(file_out, "rTheta/nt(Achaz,Taj)[%d]: %f\t", x, statistics[0].rthetaTA[x] / (double)rnsites3_pop[x]);
                            else
                                fprintf(file_out, "rTheta/nt(Achaz,Taj)[%d]: NA\n", x);
                        }
                        else
                        {
                            fprintf(file_out, "rS[%d]: NA\t", x);
            ...
            fprintf(file_out, "\nNeutrality tests (unnormalized) using RELATIVE variability for each population:\n");
            if (outgroup_presence == 1)
            {
                np = args->npops - 1;
                for (x = 0; x < np; x++)
                {
                    if (statistics[0].rDtaj[x] > -10000)
                        fprintf(file_out, "rTajima D[%d]: %f\t", x, statistics[0].rDtaj[x]);
                    else
                        fprintf(file_out, "rTajima D[%d]: NA\t", x);
                    if (statistics[0].rDfl[x] > -10000)
                        fprintf(file_out, "rFu&Li D[%d]: %f\t", x, statistics[0].rDfl[x]);
                    else
                        fprintf(file_out, "rFu&Li D[%d]: NA\t", x);
                    if (statistics[0].rFfl[x] > -10000)
                        fprintf(file_out, "rFu&Li F[%d]: %f\t", x, statistics[0].rFfl[x]);
                    else
                        fprintf(file_out, "rFu&Li F[%d]: NA\t", x);
                    if (statistics[0].rHnfw[x] > -10000)
                        fprintf(file_out, "rFay&Wu H[%d]: %f\t", x, statistics[0].rHnfw[x]);
                    else
                        fprintf(file_out, "rFay&Wu H[%d]: NA\t", x);
                    if (statistics[0].rEz[x] > -10000)
                        fprintf(file_out, "rZeng E[%d]: %f\t", x, statistics[0].rEz[x]);
                    else
                        fprintf(file_out, "rZeng E[%d]: NA\t", x);
                    if (statistics[0].rYach[x] > -10000)
                        fprintf(file_out, "rAchaz Y[%d]: %f\t", x, statistics[0].rYach[x]);
                    else
                        fprintf(file_out, "rAchaz Y[%d]: NA\t", x);
                    /**/
                    if (statistics[0].rFH[x] > -10000)
                        fprintf(file_out, "rFerretti L[%d]: %f\t", x, statistics[0].rFH[x]);
                    else
                        fprintf(file_out, "rFerretti L[%d]: NA\t", x);
                    /**/
                    fprintf(file_out, "\n");
                }
            }
            ...
            fprintf(file_out, "\nNeutrality tests / sqrt(S) using RELATIVE variability for each population:\n");
            if (outgroup_presence == 1)
            {
                np = args->npops - 1;
                for (x = 0; x < np; x++)
                {
                    if (statistics[0].rDtaj[x] > -10000)
                        fprintf(file_out, "rTajima D/sqrt(S)[%d]: %f\t", x, statistics[0].rDtaj[x]/sqrt(statistics[0].rSo[x]));
                    else
                        fprintf(file_out, "rTajima D/sqrt(S)[%d]: NA\t", x);
                    if (statistics[0].rDfl[x] > -10000)
                        fprintf(file_out, "rFu&Li D/sqrt(S)[%d]: %f\t", x, statistics[0].rDfl[x]/sqrt(statistics[0].rSo[x]));
                    else
                        fprintf(file_out, "rFu&Li D/sqrt(S)[%d]: NA\t", x);
                    if (statistics[0].rFfl[x] > -10000)
                        fprintf(file_out, "rFu&Li Fsqrt(S)[%d]: %f\t", x, statistics[0].rFfl[x]/sqrt(statistics[0].rSo[x]));
                    else
                        fprintf(file_out, "rFu&Li F/sqrt(S)[%d]: NA\t", x);
                    if (statistics[0].rHnfw[x] > -10000)
                        fprintf(file_out, "rFay&Wu H/sqrt(S)[%d]: %f\t", x, statistics[0].rHnfw[x]/sqrt(statistics[0].rSo[x]));
                    else
                        fprintf(file_out, "rFay&Wu H/sqrt(S)[%d]: NA\t", x);
                    if (statistics[0].rEz[x] > -10000)
                        fprintf(file_out, "rZeng E/sqrt(S)[%d]: %f\t", x, statistics[0].rEz[x]/sqrt(statistics[0].rSo[x]));
                    else
                        fprintf(file_out, "rZeng E/sqrt(S)[%d]: NA\t", x);
                    if (statistics[0].rYach[x] > -10000)
                        fprintf(file_out, "rAchaz Y/sqrt(S)[%d]: %f\t", x, statistics[0].rYach[x]/sqrt(statistics[0].rSo[x]));
                    else
                        fprintf(file_out, "rAchaz Y/sqrt(S)[%d]: NA\t", x);
                    /**/
                    if (statistics[0].rFH[x] > -10000)
                        fprintf(file_out, "rFerretti L/sqrt(S)[%d]: %f\t", x, statistics[0].rFH[x]/sqrt(statistics[0].rSo[x]));
                    else
                        fprintf(file_out, "rFerretti L/sqrt(S)[%d]: NA\t", x);
                    /**/
                    fprintf(file_out, "\n");
                }
            }
            else
            {
                ...
                {
                    if (statistics[0].rDtaj[x] > -10000)
                        fprintf(file_out, "rTajima D/sqrt(S)[%d]: %f\t", x, statistics[0].rDtaj[x]/sqrt(statistics[0].rS[x]));
                    else
                        fprintf(file_out, "rTajima D/sqrt(S)[%d]: NA\t", x);
                    if (statistics[0].rDfl[x] > -10000)
                        fprintf(file_out, "rFu&Li D*/sqrt(S)[%d]: %f\t", x, statistics[0].rDfl[x]/sqrt(statistics[0].rS[x]));
                    else
                        fprintf(file_out, "rFu&Li D*/sqrt(S)[%d]: NA\t", x);
                    if (statistics[0].rFfl[x] > -10000)
                        fprintf(file_out, "rFu&Li F*/sqrt(S)[%d]: %f\t", x, statistics[0].rFfl[x]/sqrt(statistics[0].rS[x]));
                    else
                        fprintf(file_out, "rFu&Li F*/sqrt(S)[%d]: NA\t", x);
                    if (statistics[0].rYach[x] > -10000)
                        fprintf(file_out, "rAchaz Y*/sqrt(S)[%d]: %f\t", x, statistics[0].rYach[x]/sqrt(statistics[0].rS[x]));
                    else
                        fprintf(file_out, "rAchaz Y*/sqrt(S)[%d]: NA\t", x);
                    fprintf(file_out, "\n");
                }
        ...
        if(args->output == 10 && args->include_rsfs==0)
        {
            if (args->npops - !outgroup_presence == 1)
                oo = 0;
            else
            ...
            for (x = 0; x < args->npops - oo; x++)
            {
                fprintf(file_out, "\npop[%d:%ld combinations]:", x, (long int)(args->vint_perpop_nsam[x]) * (args->vint_perpop_nsam[x] - 1) / 2);
                for (zz = 0; zz < args->vint_perpop_nsam[x] * (args->vint_perpop_nsam[x] - 1) / 2; zz++)
                {
                    if (ploidy[0] == '1')
                    {
                        fprintf(file_out, "\t%.2f", statistics[0].mdw[x][zz]);
                    }
                    if (ploidy[0] == '2')
                    {
                        fprintf(file_out, "\tNA");
                        ...
                        for(z1=1;z1<=(long int)floor(vint_perpop_nsam[x]/2);z1++) {
                            ...
                            for(z2=1;z2<=(long int)floor(vint_perpop_nsam[x]/2);z2++) {
                                ...
                                for(ss=initsq1[x];ss<initsq1[x]+vint_perpop_nsam[x];ss++) {
                                    mean_z1 = mean_z2 = statistics[0].thetaS[x]/vint_perpop_nsam[x];
                    ...
                    for(x=0;x<npops-oo;x++) {
                        ...
                        for(z1=1;z1<vint_perpop_nsam[x];z1++) {
                            ...
                            for(z2=1;z2<vint_perpop_nsam[x];z2++) {
                                cv = 0.;
                                for(ss=initsq1[x];ss<initsq1[x]+vint_perpop_nsam[x];ss++) {
                                    cv += (statistics[0].linefreq[ss][z1] - (statistics[0].thetaS[x]/vint_perpop_nsam[x])) *
                                    (statistics[0].linefreq[ss][z2] - (statistics[0].thetaS[x]/vint_perpop_nsam[x]));
                                }
                                fprintf(file_out,"\t%.3f",cv/vint_perpop_nsam[x]);
                            }
                        }
                    ...
                    for(x=0;x<npops-oo;x++) {
                        ...
                        for(z1=1;z1<=(long int)floor(vint_perpop_nsam[x]/2);z1++) {
                            ...
                            for(z2=1;z2<=(long int)floor(vint_perpop_nsam[x]/2);z2++) {
                                cv = 0.;
                                for(ss=initsq1[x];ss<initsq1[x]+vint_perpop_nsam[x];ss++) {
                                    mean_z1 = statistics[0].freq[x][z1] * z1/vint_perpop_nsam[x];
                                    if(!((float)z1 == floor(vint_perpop_nsam[x]/2) && (float)z1 == (float)vint_perpop_nsam[x]/2.))
                                        mean_z1 += statistics[0].freq[x][z1] * z1/vint_perpop_nsam[x];
                                    mean_z2 = statistics[0].freq[x][z2] * z2/vint_perpop_nsam[x];
                                    if(!((float)z2 == floor(vint_perpop_nsam[x]/2) && (float)z2 == (float)vint_perpop_nsam[x]/2.))
                                        mean_z2 += statistics[0].freq[x][z2] * z2/vint_perpop_nsam[x];
                    ...
                    for(x=0;x<npops-oo;x++) {
                        ...
                        for(z1=1;z1<vint_perpop_nsam[x];z1++) {
                            ...
                            for(z2=1;z2<vint_perpop_nsam[x];z2++) {
                                cv = 0.;
                                for(ss=initsq1[x];ss<initsq1[x]+vint_perpop_nsam[x];ss++) {
                                    cv += (statistics[0].linefreq[ss][z1] - (statistics[0].freq[x][z1] * z1/vint_perpop_nsam[x])) *
                                    (statistics[0].linefreq[ss][z2] - (statistics[0].freq[x][z2] * z2/vint_perpop_nsam[x]));
                                }
                                fprintf(file_out,"\t%.3f",cv/vint_perpop_nsam[x]);
                            }
                        }
    ...
    { /*single stdout line*/
        ...
        { /* SweepFinder Format */
            ...
            for (zz = 0; zz < length_seg; zz++)
            {
                if ((int)(jfd[0][zz] * nfd[0][zz]) < nfd[0][zz] && nfd[0][zz] > 1)
                {
                    fprintf(file_out, "%ld", labs(matrix_pos[zz]));
                    fprintf(file_out, "\t");
                    fprintf(file_out, "%.0f", jfd[0][zz] * nfd[0][zz]);
                    fprintf(file_out, "\t");
        ...
        else
        {
            if (args->output == 6)
            {
                ...
                for (x = 0; x < args->npops - !outgroup_presence; x++)
                {
                    for (y = 0; y < args->vint_perpop_nsam[x]; y++)
                    {
                        fprintf(file_out, " ind%03d%03d", x, y);
                    }
                }
                fprintf(file_out, "\n");
                if (args->force_outgroup == 1 || outgroup_presence == 0)
                    oo = 1;
                else
                    oo = 0;
                /*table*/
                for (zz = 0; zz < length_seg; zz++)
                {
                    ...
                    /*
                     ...
                     for(y=sumnsam-vint_perpop_nsam[npops-1];y<sumnsam;y++) {
                     if(matrix_pol[zz*sumnsam+y] == '0') {
                     freqo[1] += 1;freqo[0] += 1;
                     }
                     if(matrix_pol[zz*sumnsam+y] == '1') {
            ...
            else
            {
                if (args->output == 3)
                {
                    ...
                    for (zz = 0; zz < length_seg; zz++)
                    {
                        ...
                        if (ss && sf > 0.0 && sf < 1.0 * (args->npops - oo))
                        {
                            fprintf(file_out, "\n");
                            /*Define nucleotide*/
                            for (y = 0; y < sumnsam; y++)
                            ...
                            fprintf(file_out, "%ld\t", labs(matrix_pos[zz]));
                            if (*args->file_in)
                                fprintf(file_out, "%s\t", args->file_in);
                        }
                    }
                    fprintf(file_out, "\n");
                }
                else
                {
                    if (file_out == 0)
                        file_out = stdout;
                    if (*args->file_in)
                        fprintf(file_out, "infile:\t%s\t", args->file_in);
                    else
                        fprintf(file_out, "infile:\tNA\t");
                    if (args->gfffiles == 1)
                    {
                        fprintf(file_out, "GFFfile:\t%s\t", args->file_GFF);
                        fprintf(file_out, "subset_pos:\t%s\t", args->subset_positions);
                        if (strcmp(args->subset_positions, "synonymous") == 0 || strcmp(args->subset_positions, "nonsynonymous") == 0 || strcmp(args->subset_positions, "silent") == 0)
                            fprintf(file_out, "genetic_code:\t%s\t", args->code_name);
                    }
                    /**/
                    if (npriors)
                    {
                    ...
                    if (H1frq)
                    {
                        fprintf(file_out, "H1file:\t%s\t", args->file_H1f);
                        if (H0frq)
                        {
                            fprintf(file_out, "H0file:\t%sºt", args->file_H0f);
                        }
                    }
                    fprintf(file_out, "missing:\t%d\t", args->include_unknown);
                    fprintf(file_out, "iteration:\t%ld\t", niterdata);
                    fprintf(file_out, "npermutations:\t%ld\t", args->niter);
                    if (args->formatfile == 0 || args->formatfile == 3)
                        fprintf(file_out, "seed:\t%ld\t", args->nseed);
                    else
                        fprintf(file_out, "seed:\tna\t");
                    if (args->formatfile == 0 || args->formatfile == 3)
                        fprintf(file_out, "Length:\t%.2f\t", length_al);
                    else
                        fprintf(file_out, "Length:\t%.2f\t", length_al);
                    fprintf(file_out, "Lengtht:\t%ld\t", length_al_real);
                    fprintf(file_out, "mh:\t%ld\t", statistics[0].nmhits);
                    if (svratio > -10000)
                        fprintf(file_out, "Ratio_S/V:\t%.3f\t", svratio);
                    else
                        fprintf(file_out, "Ratio_S/V:\tNA\t");
                    if (args->include_unknown == 1)
                        fprintf(file_out, "Ratio_Missing:\t%f\t", missratio);
                    else
                        fprintf(file_out, "Ratio_Missing:\tNA\t");
                    fprintf(file_out, "Variants:\t%ld\t", length_seg);
                    
                    fprintf(file_out, "npops:\t%d\t", args->npops - !outgroup_presence);
                    for (x = 0; x < args->npops - !outgroup_presence; x++)
                    {
                        fprintf(file_out, "nsam[%d]:\t%d\t", x, args->vint_perpop_nsam[x]);
                    }
                    for (x = 0; x < args->npops - 1; x++)
                    {
                        ...
                        if (outgroup_presence == 1)
                        {
                            fprintf(file_out, "Eff_length1_pop_outg[%d]:\t%.2f\tEff_length2_pop_outg[%d]:\t%.2f\tEff_length3_pop_outg[%d]:\t%.2f\t", x, (double)nsites1_pop_outg[x], x, (double)nsites2_pop_outg[x], x, (double)nsites3_pop_outg[x]);
                        }
                        if (outgroup_presence == 0)
                        {
                            fprintf(file_out, "Eff_length1_pop[%d]:\t%.2f\tEff_length2_pop[%d]:\t%.2f\tEff_length3_pop[%d]:\t%.2f\t", x, (double)nsites1_pop[x], x, (double)nsites2_pop[x], x, (double)nsites3_pop[x]);
                        }
                    }
                    if(args->include_rsfs) {
                        for (x = 0; x < args->npops - 1; x++)
                        {
                            if (outgroup_presence == 1)
                            {
                                fprintf(file_out, "rEff_length1_pop_outg[%d]:\t%.2f\trEff_length2_pop_outg[%d]:\t%.2f\trEff_length3_pop_outg[%d]:\t%.2f\t", x, (double)rnsites1_pop_outg[x], x, (double)rnsites2_pop_outg[x], x, (double)rnsites3_pop_outg[x]);
                            }
                            if (outgroup_presence == 0)
                            {
                                fprintf(file_out, "rEff_length1_pop[%d]:\t%.2f\trEff_length2_pop[%d]:\t%.2f\trEff_length3_pop[%d]:\t%.2f\t", x, (double)rnsites1_pop[x], x, (double)rnsites2_pop[x], x, (double)rnsites3_pop[x]);
                            }
                        }
                    }
                        ...
                        if (ploidy[0] == '1')
                        {
                            if (outgroup_presence == 0)
                            {
                                for (x = 0; x < args->npops - oo; x++)
                                {
                                    ...
                                    for (z1 = 1; z1 <= (long int)floor(args->vint_perpop_nsam[x] / 2); z1++)
                                    {
                                        for (z2 = 1; z2 <= (long int)floor(args->vint_perpop_nsam[x] / 2); z2++)
                                        {
                                            cv = 0.;
                                            for (ss = initsq1[x]; ss < initsq1[x] + args->vint_perpop_nsam[x]; ss++)
                                            {
                                                mean_z1 = mean_z2 = 0.;
                                                if (z1 == 1)
                                                    mean_z1 = statistics[0].thetaT[x] / 2.;
                                                if (z2 == 1)
                                                    mean_z2 = statistics[0].thetaT[x] / 2.;
                                                
                                                cv += (statistics[0].linefreq[ss][z1] - mean_z1) *
                                                (statistics[0].linefreq[ss][z2] - mean_z2);
                                            }
                                            cv = cv / (double)args->vint_perpop_nsam[x];
                                            ...
                                            if (statistics[0].freq[x][z2] * statistics[0].freq[x][z1] > 0.)
                                            {
                                                if (z1 == z2)
                                                    ...
                                                    delta = 0.;
                                                cv = cv - delta * (double)z1 / (double)args->vint_perpop_nsam[x] * (1. - (double)z1 / (double)args->vint_perpop_nsam[x]) * statistics[0].freq[x][z1];
                            ...
                            else
                            {
                                for (x = 0; x < args->npops - oo; x++)
                                {
                                    ...
                                    for (z1 = 1; z1 < args->vint_perpop_nsam[x]; z1++)
                                    {
                                        for (z2 = 1; z2 < args->vint_perpop_nsam[x]; z2++)
                                        {
                                            cv = 0.;
                                            for (ss = initsq1[x]; ss < initsq1[x] + args->vint_perpop_nsam[x]; ss++)
                                            {
                    ...
                    if (args->output == 92)
                    {
                        ...
                        if (args->include_unknown == 0)
                        {
                            if (outgroup_presence == 0)
                            {
                                fprintf(file_out, "rSFS[npops(%d)xsumnsam/2(%ld)/2]:\t", args->npops - oo, (long int)floor((sumnsam - args->vint_perpop_nsam[args->npops - 1]) / 2));
                                for (x = 0; x < args->npops - oo; x++)
                                {
                                    for (z1 = 1; z1 <= (long int)floor((sumnsam - args->vint_perpop_nsam[args->npops - 1]) / 2); z1++)
                                    {
                            ...
                            {
                                fprintf(file_out, "rSFS[npops(%d)xsumnsam-1(%d)]:\t", args->npops - oo, sumnsam - args->vint_perpop_nsam[args->npops - 1] - 1);
                                for (x = 0; x < args->npops - oo; x++)
                                {
                                    for (z1 = 1; z1 < sumnsam - args->vint_perpop_nsam[args->npops - 1]; z1++)
                                    {
                                ...
                                for (x = 0; x < args->npops - oo; x++)
                                {
                                    ...
                                    for (z1 = 1; z1 <= (long int)floor(args->vint_perpop_nsam[x] / 2); z1++)
                                    {
                                        for (z2 = 1; z2 <= (long int)floor(args->vint_perpop_nsam[x] / 2); z2++)
                                        {
                                            ...
                                            for (ss = initsq1[x]; ss < initsq1[x] + args->vint_perpop_nsam[x]; ss++)
                                            {
                                                mean_z1 = statistics[0].freq[x][z1] * (float)z1 / (float)args->vint_perpop_nsam[x];
                                                if (!((float)z1 == floor(args->vint_perpop_nsam[x] / 2) && (float)z1 == (float)args->vint_perpop_nsam[x] / 2.))
                                                    mean_z1 += statistics[0].freq[x][z1] * (float)z1 / (float)args->vint_perpop_nsam[x];
                                                mean_z2 = statistics[0].freq[x][z2] * (float)z2 / (float)args->vint_perpop_nsam[x];
                                                if (!((float)z2 == floor(args->vint_perpop_nsam[x] / 2) && (float)z2 == (float)args->vint_perpop_nsam[x] / 2.))
                                                    mean_z2 += statistics[0].freq[x][z2] * (float)z2 / (float)args->vint_perpop_nsam[x];
                                    ...
                                    for (z1 = 1; z1 <= (long int)floor(args->vint_perpop_nsam[x] / 2); z1++)
                                    {
                                        for (z2 = 1; z2 <= (long int)floor(args->vint_perpop_nsam[x] / 2); z2++)
                                        {
                                            for (ss = initsq1[x]; ss < initsq1[x] + args->vint_perpop_nsam[x]; ss++)
                                            {
                                                ...
                                                luca_cvo2 += 1 / (double)(z1 * z1 * z2 * z2) * ((statistics[0].linefreq[ss][z1] - mean_z1) * (statistics[0].linefreq[ss][z2] - mean_z2) - delta * (double)z1 * (1. - (double)z1 / (double)args->vint_perpop_nsam[x]) * statistics[0].freq[x][z1]);
                                                luca_cvo1 += 1 / (double)(z1 * z2) * ((statistics[0].linefreq[ss][z1] - mean_z1) * (statistics[0].linefreq[ss][z2] - mean_z2) - delta * (double)z1 * (1. - (double)z1 / (double)args->vint_perpop_nsam[x]) * statistics[0].freq[x][z1]);
                                                luca_cv0 += ((statistics[0].linefreq[ss][z1] - mean_z1) *
                                                             (statistics[0].linefreq[ss][z2] - mean_z2) -
                                                             delta * (double)z1 * (1. - (double)z1 / (double)args->vint_perpop_nsam[x]) * statistics[0].freq[x][z1]);
                                                ...
                                                if (z1 == z2)
                                                {
                                                    luca_cvo2d += 1 / (double)(z1 * z1 * z2 * z2) * ((statistics[0].linefreq[ss][z1] - mean_z1) * (statistics[0].linefreq[ss][z2] - mean_z2) - delta * (double)z1 * (1. - (double)z1 / (double)args->vint_perpop_nsam[x]) * statistics[0].freq[x][z1]);
                                                    luca_cvo1d += 1 / (double)(z1 * z2) * ((statistics[0].linefreq[ss][z1] - mean_z1) * (statistics[0].linefreq[ss][z2] - mean_z2) - delta * (double)z1 * (1. - (double)z1 / (double)args->vint_perpop_nsam[x]) * statistics[0].freq[x][z1]);
                                    ...
                                    luca_cvo2 = luca_cvo2 / luca_dero2 / (double)args->vint_perpop_nsam[x];
                                    luca_cvo1 = luca_cvo1 / luca_dero1 / (double)args->vint_perpop_nsam[x];
                                    luca_cv0 = luca_cv0 / luca_der0 / (double)args->vint_perpop_nsam[x];
                                    luca_cv1 = luca_cv1 / luca_der1 / (double)args->vint_perpop_nsam[x];
                                    
                                    luca_cvo2d = luca_cvo2d / luca_dero2d / (double)args->vint_perpop_nsam[x];
                                    luca_cvo1d = luca_cvo1d / luca_dero1d / (double)args->vint_perpop_nsam[x];
                                    luca_cv0d = luca_cv0d / luca_der0d / (double)args->vint_perpop_nsam[x];
                                    luca_cv1d = luca_cv1d / luca_der1d / (double)args->vint_perpop_nsam[x];
                                ...
                                for (x = 0; x < args->npops - oo; x++)
                                {
                                    ...
                                    for (z1 = 1; z1 < args->vint_perpop_nsam[x]; z1++)
                                    {
                                        for (z2 = 1; z2 < args->vint_perpop_nsam[x]; z2++)
                                        {
                                            ...
                                            for (ss = initsq1[x]; ss < initsq1[x] + args->vint_perpop_nsam[x]; ss++)
                                            {
                                                cv += (statistics[0].linefreq[ss][z1] - (statistics[0].freq[x][z1] * (float)z1 / (float)args->vint_perpop_nsam[x])) *
                                                (statistics[0].linefreq[ss][z2] - (statistics[0].freq[x][z2] * (float)z2 / (float)args->vint_perpop_nsam[x]));
                                            }
                                            cv = cv / (double)args->vint_perpop_nsam[x];
#if DENCOV_CORRECTION == 1
                                            if (statistics[0].freq[x][z2] * statistics[0].freq[x][z1] > 0.)
                                    ...
                                    for (z1 = 1; z1 < args->vint_perpop_nsam[x]; z1++)
                                    {
                                        for (z2 = 1; z2 <= args->vint_perpop_nsam[x] - z1; z2++)
                                        {
                                            for (ss = initsq1[x]; ss < initsq1[x] + args->vint_perpop_nsam[x]; ss++)
                                            {
                                                mean_z1 = (statistics[0].freq[x][z1] * (float)z1 / (float)args->vint_perpop_nsam[x]);
                                                mean_z2 = (statistics[0].freq[x][z2] * (float)z2 / (float)args->vint_perpop_nsam[x]);
                                                luca_mean_z1 = (statistics[0].freq[x][args->vint_perpop_nsam[x] - z1] * (float)(args->vint_perpop_nsam[x] - z1) / (float)args->vint_perpop_nsam[x]);
                                                luca_mean_z2 = (statistics[0].freq[x][args->vint_perpop_nsam[x] - z2] * (float)(args->vint_perpop_nsam[x] - z2) / (float)args->vint_perpop_nsam[x]);
                                                if (z1 == z2)
                                                    delta = 1.;
                                                else
                                                    delta = 0.;
                                                ...
                                                luca_cso2 += 1 / (double)(z1 * z1 * z2 * z2) * ((statistics[0].linefreq[ss][args->vint_perpop_nsam[x] - z1] - luca_mean_z1) * (statistics[0].linefreq[ss][args->vint_perpop_nsam[x] - z2] - luca_mean_z2) - delta * (double)z1 * (1. - (double)z1 / (double)args->vint_perpop_nsam[x]) * statistics[0].freq[x][args->vint_perpop_nsam[x] - z1]);
                                                luca_cso1 += 1 / (double)(z1 * z2) * ((statistics[0].linefreq[ss][args->vint_perpop_nsam[x] - z1] - luca_mean_z1) * (statistics[0].linefreq[ss][args->vint_perpop_nsam[x] - z2] - luca_mean_z2) - delta * (double)z1 * (1. - (double)z1 / (double)args->vint_perpop_nsam[x]) * statistics[0].freq[x][args->vint_perpop_nsam[x] - z1]);
                                                luca_cs0 += ((statistics[0].linefreq[ss][args->vint_perpop_nsam[x] - z1] - luca_mean_z1) *
                                                             (statistics[0].linefreq[ss][args->vint_perpop_nsam[x] - z2] - luca_mean_z2) -
                                                             delta * (double)z1 * (1. - (double)z1 / (double)args->vint_perpop_nsam[x]) * statistics[0].freq[x][args->vint_perpop_nsam[x] - z1]);
                                                luca_cs1 += (double)(z1 * z2) * ((statistics[0].linefreq[ss][args->vint_perpop_nsam[x] - z1] - luca_mean_z1) *
                                                                                 (statistics[0].linefreq[ss][args->vint_perpop_nsam[x] - z2] - luca_mean_z2) -
                                                                                 delta * (double)z1 * (1. - (double)z1 / (double)args->vint_perpop_nsam[x]) * statistics[0].freq[x][args->vint_perpop_nsam[x] - z1]);
                                                
                                                luca_cro2 += ((double)min(z1, z2) / (double)args->vint_perpop_nsam[x] - (double)(z1 * z2) / (double)(args->vint_perpop_nsam[x] * args->vint_perpop_nsam[x])) *
                                                1 / (double)(z1 * z1 * z2 * z2) * (statistics[0].freq[x][args->vint_perpop_nsam[x] - z1] * (statistics[0].freq[x][args->vint_perpop_nsam[x] - z2] - delta));
                                                luca_cro1 += ((double)min(z1, z2) / (double)args->vint_perpop_nsam[x] - (double)(z1 * z2) / (double)(args->vint_perpop_nsam[x] * args->vint_perpop_nsam[x])) *
                                                1 / (double)(z1 * z2) * (statistics[0].freq[x][args->vint_perpop_nsam[x] - z1] * (statistics[0].freq[x][args->vint_perpop_nsam[x] - z2] - delta));
                                                luca_cr0 += ((double)min(z1, z2) / (double)args->vint_perpop_nsam[x] - (double)(z1 * z2) / (double)(args->vint_perpop_nsam[x] * args->vint_perpop_nsam[x])) *
                                                (statistics[0].freq[x][args->vint_perpop_nsam[x] - z1] * (statistics[0].freq[x][args->vint_perpop_nsam[x] - z2] - delta));
                                                luca_cr1 += ((double)min(z1, z2) / (double)args->vint_perpop_nsam[x] - (double)(z1 * z2) / (double)(args->vint_perpop_nsam[x] * args->vint_perpop_nsam[x])) *
                                                (double)(z1 * z2) * (statistics[0].freq[x][args->vint_perpop_nsam[x] - z1] * (statistics[0].freq[x][args->vint_perpop_nsam[x] - z2] - delta));
                                                ...
                                                if (z1 == z2)
                                                {
                                                    ...
                                                    luca_cso2d += 1 / (double)(z1 * z1 * z2 * z2) * ((statistics[0].linefreq[ss][args->vint_perpop_nsam[x] - z1] - luca_mean_z1) * (statistics[0].linefreq[ss][args->vint_perpop_nsam[x] - z2] - luca_mean_z2) - delta * (double)z1 * (1. - (double)z1 / (double)args->vint_perpop_nsam[x]) * statistics[0].freq[x][args->vint_perpop_nsam[x] - z1]);
                                                    luca_cso1d += 1 / (double)(z1 * z2) * ((statistics[0].linefreq[ss][args->vint_perpop_nsam[x] - z1] - luca_mean_z1) * (statistics[0].linefreq[ss][args->vint_perpop_nsam[x] - z2] - luca_mean_z2) - delta * (double)z1 * (1. - (double)z1 / (double)args->vint_perpop_nsam[x]) * statistics[0].freq[x][args->vint_perpop_nsam[x] - z1]);
                                                    luca_cs0d += ((statistics[0].linefreq[ss][args->vint_perpop_nsam[x] - z1] - luca_mean_z1) *
                                                                  (statistics[0].linefreq[ss][args->vint_perpop_nsam[x] - z2] - luca_mean_z2) -
                                                                  delta * (double)z1 * (1. - (double)z1 / (double)args->vint_perpop_nsam[x]) * statistics[0].freq[x][args->vint_perpop_nsam[x] - z1]);
                                                    luca_cs1d += (double)(z1 * z2) * ((statistics[0].linefreq[ss][args->vint_perpop_nsam[x] - z1] - luca_mean_z1) *
                                                                                      (statistics[0].linefreq[ss][args->vint_perpop_nsam[x] - z2] - luca_mean_z2) -
                                                                                      delta * (double)z1 * (1. - (double)z1 / (double)args->vint_perpop_nsam[x]) * statistics[0].freq[x][args->vint_perpop_nsam[x] - z1]);
                                                    
                                                    luca_cro2d += ((double)min(z1, z2) / (double)args->vint_perpop_nsam[x] - (double)(z1 * z2) / (double)(args->vint_perpop_nsam[x] * args->vint_perpop_nsam[x])) *
                                                    1 / (double)(z1 * z1 * z2 * z2) * (statistics[0].freq[x][args->vint_perpop_nsam[x] - z1] * (statistics[0].freq[x][args->vint_perpop_nsam[x] - z2] - delta));
                                                    luca_cro1d += ((double)min(z1, z2) / (double)args->vint_perpop_nsam[x] - (double)(z1 * z2) / (double)(args->vint_perpop_nsam[x] * args->vint_perpop_nsam[x])) *
                                                    1 / (double)(z1 * z2) * (statistics[0].freq[x][args->vint_perpop_nsam[x] - z1] * (statistics[0].freq[x][args->vint_perpop_nsam[x] - z2] - delta));
                                                    luca_cr0d += ((double)min(z1, z2) / (double)args->vint_perpop_nsam[x] - (double)(z1 * z2) / (double)(args->vint_perpop_nsam[x] * args->vint_perpop_nsam[x])) *
                                                    (statistics[0].freq[x][args->vint_perpop_nsam[x] - z1] * (statistics[0].freq[x][args->vint_perpop_nsam[x] - z2] - delta));
                                                    luca_cr1d += ((double)min(z1, z2) / (double)args->vint_perpop_nsam[x] - (double)(z1 * z2) / (double)(args->vint_perpop_nsam[x] * args->vint_perpop_nsam[x])) *
                                                    (double)(z1 * z2) * (statistics[0].freq[x][args->vint_perpop_nsam[x] - z1] * (statistics[0].freq[x][args->vint_perpop_nsam[x] - z2] - delta));
                                                }
                                            }
                                        }
                                    ...
                                    luca_cvo2 = luca_cvo2 * luca_cro2 / luca_cso2 / luca_dero2 / (double)args->vint_perpop_nsam[x];
                                    luca_cvo1 = luca_cvo1 * luca_cro1 / luca_cso1 / luca_dero1 / (double)args->vint_perpop_nsam[x];
                                    luca_cv0 = luca_cv0 * luca_cr0 / luca_cs0 / luca_der0 / (double)args->vint_perpop_nsam[x];
                                    luca_cv1 = luca_cv1 * luca_cr1 / luca_cs1 / luca_der1 / (double)args->vint_perpop_nsam[x];
                                    
                                    luca_cvo2d = luca_cvo2d * luca_cro2d / luca_cso2d / luca_dero2d / (double)args->vint_perpop_nsam[x];
                                    luca_cvo1d = luca_cvo1d * luca_cro1d / luca_cso1d / luca_dero1d / (double)args->vint_perpop_nsam[x];
                                    luca_cv0d = luca_cv0d * luca_cr0d / luca_cs0d / luca_der0d / (double)args->vint_perpop_nsam[x];
                                    luca_cv1d = luca_cv1d * luca_cr1d / luca_cs1d / luca_der1d / (double)args->vint_perpop_nsam[x];
                                ...
                                for (x = 0; x < args->npops - oo; x++)
                                {
                                    ...
                                    for (z1 = 1; z1 <= (long int)floor(args->vint_perpop_nsam[x] / 2); z1++)
                                    {
                                        for (z2 = 1; z2 <= (long int)floor(args->vint_perpop_nsam[x] / 2); z2++)
                                        {
                                            ...
                                            for (ss = initsq1[x]; ss < initsq1[x] + args->vint_perpop_nsam[x]; ss++)
                                            {
                                                mean_z1 = mean_z2 = statistics[0].thetaS[x] / (float)args->vint_perpop_nsam[x];
                                                if (!((float)z1 == floor(args->vint_perpop_nsam[x] / 2) && (float)z1 == (float)args->vint_perpop_nsam[x] / 2.))
                                                    mean_z1 *= 2.;
                                                if (!((float)z2 == floor(args->vint_perpop_nsam[x] / 2) && (float)z2 == (float)args->vint_perpop_nsam[x] / 2.))
                                                    mean_z2 *= 2.;
                                ...
                                for (x = 0; x < args->npops - oo; x++)
                                {
                                    ...
                                    for (z1 = 1; z1 < args->vint_perpop_nsam[x]; z1++)
                                    {
                                        for (z2 = 1; z2 < args->vint_perpop_nsam[x]; z2++)
                                        {
                                            ...
                                            for (ss = initsq1[x]; ss < initsq1[x] + args->vint_perpop_nsam[x]; ss++)
                                            {
                                                cv += (statistics[0].linefreq[ss][z1] - (statistics[0].thetaS[x] / (float)args->vint_perpop_nsam[x])) *
                                                (statistics[0].linefreq[ss][z2] - (statistics[0].thetaS[x] / (float)args->vint_perpop_nsam[x]));
                                            }
                                            cv = cv / (double)args->vint_perpop_nsam[x];
#if DENCOV_CORRECTION == 1
                                            if (statistics[0].freq[x][z2] * statistics[0].freq[x][z1] > 0.)
                    ...
                    if (args->output == 5)
                    {
                        if (ploidy[0] == '1')
                        {
                            ...
                            if (outgroup_presence == 0)
                            {
                                for (x = 0; x < args->npops - oo; x++)
                                {
                                    for (z = initsq1[x]; z < initsq1[x] + args->vint_perpop_nsam[x]; z++)
                                    {
                                        for (ss = 1; ss < (long int)floor(args->vint_perpop_nsam[x] / 2); ss++)
                                        {
                            ...
                            else
                            {
                                for (x = 0; x < args->npops - oo; x++)
                                {
                                    for (z = initsq1[x]; z < initsq1[x] + args->vint_perpop_nsam[x]; z++)
                                    {
                                        for (ss = 1; ss < args->vint_perpop_nsam[x]; ss++)
                                        {
                                            fprintf(file_out, "line_freq[%d,%d]:\t%.3f\t", z, ss, statistics[0].linefreq[z][ss]);
                                        }
                                    }
                    ...
                    if (args->output == 4)
                    {
                        ...
                        for (x = 0; x < args->npops - oo; x++)
                        {
                            fprintf(file_out, "Pairwise_Comparisons_npop[%d:%ld comb]:\t", x, (long int)(args->vint_perpop_nsam[x]) * (args->vint_perpop_nsam[x] - 1) / 2);
                            for (zz = 0; zz < args->vint_perpop_nsam[x] * (args->vint_perpop_nsam[x] - 1) / 2; zz++)
                            {
                                if (ploidy[0] == '1')
                                {
                                    fprintf(file_out, "%.1f\t", statistics[0].mdw[x][zz]);
                                }
                                if (ploidy[0] == '2')
                                {
                                    fprintf(file_out, "NA\t");
                    ...
                    if (args->output == 2)
                    {
                        ...
                        if (outgroup_presence == 1)
                        {
                            for (x = 0; x < args->npops - 1; x++) /*MOD \t 20240618*/
                            {
                                fprintf(file_out, "nsam[%d]:\t%d\tS[%d]:\t%ld\t", x, args->vint_perpop_nsam[x], x, (long int)statistics[0].S[x]);
                                for (y = 1; y < args->vint_perpop_nsam[x]; y++)
                                {
                                    fprintf(file_out, "fr[%d,%d]:\t%ld\t", x, y, statistics[0].freq[x][y]);
                                }
                        ...
                        /*fprintf(file_out,"\n");
                         ...
                         if(npops>1) {
                         for(x=0;x<npops-oo;x++) {
                         fprintf(file_out,"%d\t%ld\t",vint_perpop_nsam[x],(long int)statistics[0].S[x]);
                    ...
                    if (args->output == 1)
                    {
                        if(args->include_rsfs==0) {
                            ...
                            if (outgroup_presence + args->force_outgroup)
                            {
                                ...
                                {
                                    int nsam_x;
                                    nsam_x = args->vint_perpop_nsam[x];
                                    if (nsam_x > 1)
                                    {
                                        /*
                                         fprintf(file_out,"S[%d]:\t%d\t",x,(int)statistics[0].S[x]);
                                    ...
                                    if (args->force_outgroup == 0)
                                        fprintf(file_out, "Divergence[%d]:\t%f\t", x, statistics[0].K[x]);
                                    else
                                        fprintf(file_out, "Divergence[%d]:\tNA\t", x);
                                    if (statistics[0].KHKY[x] > -10000 /* && missratio == 0.*/)
                                        fprintf(file_out, "Divergence/nt_HKY[%d]:\t%f\t", x, statistics[0].KHKY[x]);
                                    else
                                        fprintf(file_out, "Divergence/nt_HKY[%d]:\tNA\t", x);
                                }
                            }
                            else
                            {
                                ...
                                if (outgroup_presence == 1)
                                {
                                    for (x = 0; x < args->npops - 1; x++)
                                    {
                                        ...
                                        if (statistics[0].To_Lc_ii[x] > -10000)
                                            ...
                                            fprintf(file_out, "To_Lc_ii[%d]:\tNA\t", x);
#endif
                                        if (H0frq)
                                        {
                                            if (statistics[0].ToH0_ii[x] > -10000)
                            ...
                            if (args->npops > 2 && outgroup_presence == 1 )
                            {
                                ...
                                fprintf(file_out, "Ss[outg,rest]:\t%ld\t", statistics[0].Sanc[(args->npops - 1) * 4 + 2]);
                                if (args->npops > 3)
                                    fprintf(file_out, "Ss[rest]:\t%ld\t", statistics[0].Sanc[(args->npops - 1) * 4 + 3]);
                                else
                                    fprintf(file_out, "Ss[rest]:\tNA\t");
                            }
                            else
                            ...
                            for (x = 0; x < args->npops - 1 /*!outgroup_presence*/; x++)
                            {
                                ...
                                {
                                    if (statistics[0].mdg1[x] > -10000. && statistics[0].mdsd[x] > -10000.)
                                        fprintf(file_out, "MD_Skewness[%d]:\t%f\t", x, statistics[0].mdg1[x]);
                                    else
                                ...
                                {
                                    if (statistics[0].mdg2[x] > -10000. && statistics[0].mdsd[x] > -10000.)
                                        fprintf(file_out, "MD_Kurtosis[%d]:\t%f\t", x, statistics[0].mdg2[x]);
                                    else
                            ...
                            if (ploidy[0] == '1' && args->include_unknown == 0)
                            {
                                ...
                                if (args->npops - 1 /*!outgroup_presence*/ > 1)
                                {
                                    ...
                                    if (args->npops - 1 /*!outgroup_presence*/ > 2)
                                    {
                                        ...
                                        {
                                            if (args->niter && piter[0].niterihall > 0)
                                                fprintf(file_out, "FstH:\tNA\t P-value:\tNA\t");
                                            else
                                                fprintf(file_out, "FstH:\tNA\t P-value:\tNA\t");
                                        }
                                        ...
                                        for (x = 0; x < args->npops - 1; x++)
                                        {
                                            if (statistics[0].fsth1all[x] > -10000 && args->include_unknown == 0)
                                            {
                                                if (args->niter && piter[0].niterih1[x])
                                                    fprintf(file_out, "FstH1[%d,rest]:\t%f\t P-value:\t%f\t", x, statistics[0].fsth1all[x], (double)piter[0].ih1[x] / (double)piter[0].niterih1[x]);
                                                else
                                                    fprintf(file_out, "FstH1[%d,rest]:\t%f\t P-value:\tNA\t", x, statistics[0].fsth1all[x]);
                                            }
                                            else
                                            {
                                                if (args->niter && piter[0].niterih1[x] > 0 && args->include_unknown == 0)
                                                    fprintf(file_out, "FstH1[%d,rest]:\tNA\t P-value:\tNA\t", x);
                                                else
                                                    fprintf(file_out, "FstH1[%d,rest]:\tNA\t P-value:\tNA\t", x);
                                            }
                                        }
                                    ...
                                    for (x = 0; x < args->npops - 1; x++)
                                    {
                                        for (y = x + 1; y < args->npops - 0; y++)
                                        {
                                            if (y == args->npops - 1 && !outgroup_presence)
                                            {
                                                z++;
                                                continue;
                                            }
                            ...
                            { /*
                               for(x=0;x<npops-1;x++) {
                               if(vint_perpop_nsam[x]>1) fprintf(file_out,"PiW[%d]:\t%f\t",x,statistics[0].piw[x]);
                        ...
                        /*PRINT DATA FOR theta estimates obtained with rSFS*/
                        if(args->include_rsfs==1) {
                            if (outgroup_presence + args->force_outgroup)
                            {
                                if (args->force_outgroup == 0)
                                    ...
                                    fprintf(file_out, "Divergence:\tNA\t");
                                ...
                                for (x = 0; x < np; x++)
                                {
                                    ...
                                    if (nsam_x && args->int_total_nsam>1)
                                    {
                                        fprintf(file_out, "rS[%d]:\t%.0f\t", x, statistics[0].rSo[x]);
                                        fprintf(file_out, "rTheta(Wat)[%d]:\t%f\t", x, statistics[0].rthetaSo[x]);
                                        fprintf(file_out, "rTheta(Taj)[%d]:\t%f\t", x, statistics[0].rthetaTo[x]);
                            ...
                            if (outgroup_presence + args->force_outgroup)
                            {
                                ...
                                {
                                    if (statistics[0].rDtaj[x] > -10000)
                                        fprintf(file_out, "rTajimaD[%d]:\t%f\t", x, statistics[0].rDtaj[x]);
                                    else
                                        fprintf(file_out, "rTajimaD[%d]:\tNA\t", x);
                                    if (statistics[0].rDfl[x] > -10000)
                                        fprintf(file_out, "rFu&LiD[%d]:\t%f\t", x, statistics[0].rDfl[x]);
                                    else
                                        fprintf(file_out, "rFu&LiD[%d]:\tNA\t", x);
                                    if (statistics[0].rFfl[x] > -10000)
                                        fprintf(file_out, "rFu&LiF[%d]:\t%f\t", x, statistics[0].rFfl[x]);
                                    else
                                        fprintf(file_out, "rFu&LiF[%d]:\tNA\t", x);
                                    if (statistics[0].rHnfw[x] > -10000)
                                        fprintf(file_out, "rFay&WuH[%d]:\t%f\t", x, statistics[0].rHnfw[x]);
                                    else
                                        fprintf(file_out, "rFay&WuH[%d]:\tNA\t", x);
                                    if (statistics[0].rEz[x] > -10000)
                                        fprintf(file_out, "rZengE[%d]:\t%f\t", x, statistics[0].rEz[x]);
                                    else
                                        fprintf(file_out, "rZengE[%d]:\tNA\t", x);
                                    if (statistics[0].rYach[x] > -10000)
                                        fprintf(file_out, "rAchazY[%d]:\t%f\t", x, statistics[0].rYach[x]);
                                    else
                                        fprintf(file_out, "rAchazY[%d]:\tNA\t", x);
                                    /**/
                                    if (statistics[0].rFH[x] > -10000)
                                        fprintf(file_out, "rFerrettiL[%d]:\t%f\t", x, statistics[0].rFH[x]);
                                    else
                                        fprintf(file_out, "rFerrettiL[%d]:\tNA\t", x);
                                    /**/
                                }
                            }
                            else
