...
int get_obsstats(FILE *file_output,FILE *file_mask,int n_samp, long int n_site,
				 ...
				 double *anx, double *bnx,
				 double *anxo, double *bnxo,double **lengthamng,double **lengthamng_outg,
				 long int *mhitbp,char **matrix_pol_tcga,long int formatfile)
{
	/*int *unic = 0;*/
	/*long int mhits = 0;*/
    /*long int *mhitbp = 0;*/
	...
	*/
	...
	if(n_samp < 2) {
		printf(" n_samples: %d .",n_samp);
		printf(" NOT ENOUGH SAMPLES.");
		if(file_output) {
			fprintf(file_output," n_samples: %d .",n_samp);
        ...
        *n_realsite += (long int)1;
        ...
        do {
            ...
            if(w > 48 + 4) { /* the position y (also y == 0) is not a, g, t or c */
                mis += 1;
                if(y>=n_samp-nsamuser[npops-1]) miso += 1;
        ...
        if(z==0 /*valid position*/ && v == 1/*no mhit*/ && miso < nsamuser[npops-1]/*outgroup present*/) {
            ...
            if(include_unknown == 1) {
				...
				if(outgroup_presence) {
					for(y=nsamtot-nsamuser[npops-1];y<nsamtot;y++) {
				...
				if(outgroup_presence==1 && (aloutg == -1 /*all Ns*/|| fal[aloutg]+fal[4] < nsamuser[npops-1] /*outg polym*/)) {
					/*reject*/
					algsites -= matrix_sizepos[xx];
                    *n_realsite -= (long int)1;
					...
					fal[0]=fal[1]=fal[2]=fal[3]=fal[4]=0;
					for(y=0;y<nsamtot-nsamuser[npops-1];y++) {
						a = (int)*(DNA_matr+(((long long)n_site*(unsigned long)y)+(unsigned long)xx));
						switch(a) {
							case 48+1:
								fal[0]+= 1;
			...
			} else { /*mhits excluded when missing data is not allowed*/
                *n_realsite -= (long int)1;
				algsites -= matrix_sizepos[xx];
				z = 1;
			}
        ...
        if(z==0 && v==0 && miso < nsamuser[npops-1] && m_1+mis<nsamtot/*n_samp*/ && matrix_segrpos[xx]/*to eliminate biallelic syn/nsyn not desired*/) {
			...
			*/
			...
			for(y=0;y<nsamtot/*n_samp*/;y++) { /* non-outgroup: 0 indicates higher frequency */
                if(*(DNA_matr+(((long long)n_site*(unsigned long)y)+(unsigned long)xx)) == k_0) {
					...
					if(*(DNA_matr+(((long long)n_site*(unsigned long)y)+(unsigned long)xx)) == 48+5 ||
                       *(DNA_matr+(((long long)n_site*(unsigned long)y)+(unsigned long)xx)) == 48+6) {/*6 addded*/
        ...
        if(include_unknown == 1 ) {/*redundant conditional? (unknown==1 vs mis>0 or miso>0)*/
            if(z==0 && (mis == nsamtot || miso == nsamuser[npops-1])) {
                *n_realsite -= (long int)1;
                _sites -= matrix_sizepos[xx];
                algsites -= matrix_sizepos[xx];
            }
	...
	*/
		...
		if(output == 0 || output == 10) {
			...
			*/
			if(*nmhits) {
				...
				for(x=0;x<*nmhits;x++) {
					if(formatfile>0) printf(" %ld",mhitbp[x] + formatfile-1);
	...
	if(_sites == (double)0) {
        ...
        /*
        if(file_output)
			fprintf(file_output,"Not valid sites available ");
		else
		...
		if(/*include_unknown && */file_mask){
			...
			for(xx=0;xx<n_site;xx++) {
                ...
                if(include_unknown == 0 && mhitbp[xm]==xx+1)
		...
		if(include_unknown) {
			...
			for(y=0;y<npops;y++) {
				...
				for(xx=0;xx<n_site;xx++) {
					...
					if((matrix_sizepos[xx] > 0/* && mhitbp[xm] != xx+1*/)/* && ((include_unknown == 1) || (include_unknown==0 && v == 0))*/) {
						if(v < nsamuser[y]) nsites1_pop[y] += (matrix_sizepos[xx]);
						if(v < nsamuser[y]-1) nsites2_pop[y] += (matrix_sizepos[xx]);
						if(v < nsamuser[y]-2) nsites3_pop[y] += (matrix_sizepos[xx]);
						...
						if(outgroup_presence == 1) {
							...
							if(p < nsamuser[npops-1] && v < nsamuser[y]/* && mhitbp[xm] != xx+1*/) /*{*/
                                nsites1_pop_outg[y] += (matrix_sizepos[xx]);/*printf("%ld ",xx);}*/
							if(p < nsamuser[npops-1] && v < nsamuser[y]-1/* && mhitbp[xm] != xx+1*/)
								nsites2_pop_outg[y] += (matrix_sizepos[xx]);
							if(p < nsamuser[npops-1] && v < nsamuser[y]-2/* && mhitbp[xm] != xx+1*/)
								nsites3_pop_outg[y] += (matrix_sizepos[xx]);
							if(p < nsamuser[npops-1]) {
								for(k=1;k<nsamuser[y]-v;k++) {
									anxo[y] += 1.0/((double)k);
			...
			for(xx=0;xx<n_site;xx++) {
				...
				for(y=0;y<npops-1;y++) {
					...
					for(y2=y+1;y2<npops;y2++) {
						...
						while(mhitbp[xm]<xx+1 && xm<*nmhits) {xm++;} 
						p = 0;
						if((/*include_unknown == 1 && */matrix_sizepos[xx] > 0)/* || (include_unknown==0 && v == 0 && matrix_sizepos[xx] > 0)*/) {
							if(outgroup_presence == 1) {
								...
								if(p < nsamuser[npops-1] && v < nsamuser[y] && v2 < nsamuser[y2]/* && mhitbp[xm] != xx+1*/)
									lengthamng_outg[y][y2] += (matrix_sizepos[xx]);
							}
                            if(v < nsamuser[y] && v2 < nsamuser[y2]/* && mhitbp[xm] != xx+1*/)
			...
			for(y=0;y<npops;y++) {
				for(k=1;k<nsamuser[y];k++) {
					anx[y] += 1.0/((double)k);
					bnx[y] += 1.0/((double)k*(double)k);
				}
				if(nsamuser[y]>0) nsites1_pop[y] = algsites;
				if(nsamuser[y]>1) nsites2_pop[y] = algsites;
				if(nsamuser[y]>2) nsites3_pop[y] = algsites;
				for(y2=y+1;y2<npops;y2++) {
				...
				if(outgroup_presence == 1) {
					...
					if(nsamuser[y]>0) nsites1_pop_outg[y] = algsites;
					if(nsamuser[y]>1) nsites2_pop_outg[y] = algsites;
					if(nsamuser[y]>2) nsites3_pop_outg[y] = algsites;
				}
			}
		}
    ...
    *length_seg = bial_sites;
	...
	if(((double)(n_samp-nsamuser[npops-1])*n_realsite[0]))
        *missratio = (double)mis2/((double)(n_samp-nsamuser[npops-1])*n_realsite[0]);
    else
        *missratio = 1.0;
